...
class AdminFoodLabelController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $record = FoodLabelValidation::where('id', $id)->where('is_deleted', 0)->firstOrFail();
    ...
    public function download($id)
    {
        ...
        if (!$record->final_label_path) abort(404, 'No label file available.');
    ...
    // Admin download lab report uploaded by user
    public function labReportDownload($id)
    {
        $record = FoodLabelValidation::where('id', $id)->where('is_deleted', 0)->firstOrFail();

        if (!$record->lab_report_path) abort(404, 'No lab report available.');
